# Enhancing the Difference-Aware Medical Visual Question Answering with Cross-modal Prototype Representation Learning

This is the official implementation of [Enhancing the Difference-Aware Medical Visual Question Answering with Cross-modal Prototype Representation Learning]
## Abstract
...
```bash
conda env create --name DVQACPR --file env.yml
```
...
```
python -m torch.distributed.launch --use_env --nproc_per_node=1 --nnodes=2 main.py
```
## Debug this file
```python -m torch.distributed.launch --use_env --nproc_per_node=1 --nnodes=1 main.py
```
